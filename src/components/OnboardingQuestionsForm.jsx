...
const OnboardingQuestionsForm = () => {
  ...
  const initialAnswers = questions.reduce((acc, q) => ({ ...acc, [q.id]: '' }), {});
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        ...
        toast.error(data.message || 'Failed to save answers. Please try again.');
      }
    } catch (err) {
  ...
  return (
    <div className="w-full max-w-lg p-6 sm:p-8 bg-white shadow-xl rounded-lg border border-gray-200">
      <h2 className="text-2xl font-bold text-center text-gray-800 mb-2">A Few More Questions...</h2>
